...
import (
	...
	sample "github.com/synerex/proto_sample"
...
var (
	fmsrv           = flag.String("fmsrv", "wss://fm.synergic.mobi:8443/", "SampleManager Server")
	hostName        = flag.String("host", "", "sample provider host name")
...
type MyVehicle struct {
	vehicles []*MySample `json:"vehicles"`
}
...
func publishSupplyFromSampleManager(client *sxutil.SXServiceClient, ch chan error) {
	// Connect by SocketIO
	...
	sioClient.On(gosocketio.OnConnection, func(c *gosocketio.Channel, param interface{}) {
		log.Printf("Sample-Provider socket.io connected %v", c)
	})
	sioClient.On(gosocketio.OnDisconnection, func(c *gosocketio.Channel, param interface{}) {
		log.Printf("Sample-Provider socket.io disconnected %v", c)
...
func runPublishSupplyInfinite(sclient *sxutil.SXServiceClient) {
	ch := make(chan error)
	for {
		publishSupplyFromSampleManager(sclient, ch)
...
func main() {
fmt.Println("Hello!")
	log.Printf("SampleProvider(%s) built %s sha1 %s", sxutil.GitVer, sxutil.BuildTime, sxutil.Sha1Ver)
	flag.Parse()
	...
	argJson := fmt.Sprintf("{Client:Sample}")
	sclient := sxutil.NewSXServiceClient(client, pbase.RIDE_SHARE, argJson)

	wg.Add(1)

	// We add Sample Provider to "RIDE_SHARE" Supply
